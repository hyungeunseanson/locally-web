...
- 게스트/호스트 대시보드 연동 다국어 확대(v3.39.31): 로컬리 기본 프로필 메뉴 및 프로필 모달뷰(`UserProfileModal`), 호스트 대시보드 컴포넌트(`ReservationManager`), 게스트 대시보드 컴포넌트(`GuestTrips`, `CancellationModal`, `ReceiptModal` 등), 위시리스트 내 하드코딩되었던 한국어 문자열(알림 토스트 메시지, 달력 추가 정보, '취소 규정 요약', 결제 영수증 등)을 `LanguageContext` 중앙 사전에 4개 국어(ko, en, ja, zh)로 보충 등록하고 핀셋 치환함. 관련 컴포넌트들의 `useGuestTrips.ts` 훅 메시지도 모두 번역 적용. 기존 작동 논리 회귀 없음.
