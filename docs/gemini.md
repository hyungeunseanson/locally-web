...
**Last Updated:** 2026-04-15 (v3.40.18 admin support unread alert delay)
**Version:** 3.40.18 (Admin Support Unread Alert Delay)
...
- `/api/admin/users/[userId]/timeline`: User Management 상세 패널용 회원 활동 타임라인 API (예약/리뷰/문의 진행/맞춤 의뢰 상태 이벤트 조립 + `guest_reviews` read-only 상세 데이터 포함)
...
- `guest_reviews`는 `/api/admin/reviews`나 Data Analytics `Review Quality`에 섞지 않는다. 관리자 노출은 `/api/admin/users/[userId]/timeline`과 `Users` 상세 패널 read-only surface로만 유지하고, 별도 moderation/delete semantics는 정의하지 않는다.
...
- **[고객센터 1:1 문의 unread 알림 결정]** `admin_support/admin` 문의에서 고객이 보낸 메시지는 즉시 관리자 메일을 보내지 않는다. 대신 `admin_support_unread_alert_batches` 배치를 만들고, 관리자가 읽지 않은 상태가 1시간 유지되면 `admin_alert` + 팀 메일을 1회만 발송한다. 읽음 기준은 `GET /api/admin/inquiries/[id]/messages`가 서버에서 실행하는 `markInquiryMessagesRead()`다.
...
- 댓글 좋아요는 더 이상 공개 커뮤니티 기능이 아니다. 댓글 조회는 `community_comment_likes`를 읽지 않고, `/api/community/comment-likes`는 항상 `410 Gone`으로 종료된 기능임을 반환한다. 게시글 좋아요와 댓글/답글 작성 기능은 그대로 유지한다.
...
- 전화 예약 운영 답글의 source of truth는 `/api/proxy-bookings/[id]/comments`다. 현재 route는 linked inquiry가 연결된 요청만 허용하고, 답글 저장과 알림 흐름은 기존 `inquiries / inquiry_messages` 엔진을 그대로 재사용한다.
...
- `TEAM > 전화 예약`은 계속 `proxy_requests`를 source of truth로 쓰되, 대화는 기존 `inquiries / inquiry_messages`만 읽고 쓴다. 즉 전화 예약 탭은 요청 보드이고, 실제 고객 대화 엔진은 기존 문의함 하나로 통일한다.
...
- 구성요소 전체 다국어 완료(v3.39.31): `UserProfileModal`, `MobileProfileView`, `GuestProfileModal`, `ReservationManager`, `Earnings`, `GuestTrips`, `CancellationModal`, `ReceiptModal`, `useGuestTrips.ts`, `wishlists/page.tsx`, `SiteHeader.tsx`(호스트메뉴 9개), `HostProfileCard.tsx`, `ServiceCard.tsx`, `ReviewModal.tsx` 등 어드민 제외 모든 컴포넌트 내 하드코딩된 한국어 문자열을 `LanguageContext` 중앙 사전에 4개 국어(ko, en, ja, zh)로 추가하고 핀셋 `t()` 치환 완료. 레이아웃·작동 로직 회귀 없음.
- 최소 Sentry 에러 추적 도입(v3.39.32): 루트 `instrumentation.ts` / `instrumentation-client.ts`와 기존 `app/error.tsx`, `app/experiences/[id]/error.tsx`, 핵심 예약/결제/알림 API catch에만 에러 캡처를 연결했다. `NEXT_PUBLIC_SENTRY_DSN` 또는 `SENTRY_DSN`이 없으면 완전 no-op이며, tracing/replay/source map 업로드는 보류한다. `app/layout.tsx`, `app/middleware.ts`, Supabase auth/session, realtime 구조는 변경하지 않는다.
...
- 호스트 대시보드 `Earnings`는 상단 `통합 정산 대기 총액`과 하단 `체험 상세 / 서비스 상세` 드릴다운 구조를 사용한다. 메인 요약은 `GET /api/host/earnings/summary`가 단일 source of truth이며, 프론트는 체험/서비스 금액을 직접 더하지 않는다. 체험/서비스 상세는 각각의 정산 상태를 유지하되, 메인 총액에는 `pending`만 포함하고 `in_progress`, `paid`는 별도 분해로만 노출한다.
...
- `/search`의 시간대/유형 필터도 서버가 단일 source다. `SearchTimeId`, `SearchTypeId`, `SEARCH_TIME_RANGES`, `SEARCH_TYPE_KEYWORDS` 계약을 `app/search/searchContract.ts`에 두고, page는 선택 상태를 query로만 넘기며 실제 결과는 `/api/search/experiences` 응답을 그대로 렌더한다.
...
- 커뮤니티 목록 카드 렌더는 상세용 `CommunityPost`를 그대로 공유하지 않는다. `app/community/feedSelect.ts`의 `CommunityFeedPost/Profile/Experience` 타입과 `buildCommunityFeedPosts()` mapper를 통해 page/API/card가 같은 feed 전용 조립 경로를 사용한다.
