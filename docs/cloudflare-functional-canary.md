...
For Google/Kakao, a human signs in to Access first and retains the canary-origin `CF_Authorization` cookie while the browser visits Supabase and the provider, then returns through the staging Supabase allowlisted canary `/auth/callback`. Automation adds `CF-Access-Client-Id` and `CF-Access-Client-Secret` only while continuing requests whose URL origin exactly equals the canary origin. Redirect-created requests to Supabase or a provider do not match that route and cannot inherit the Access headers. Every APIRequestContext call carrying the token also disables redirects. Never use Playwright `extraHTTPHeaders`, never attach these headers to Supabase/provider requests, keep traces disabled while a service token is loaded, and retain the OAuth assertion that both headers are absent.
...
- `CLOUDFLARE_CANARY_EXPERIENCE_ID`
...
5. Set `CLOUDFLARE_FUNCTIONAL_CANARY_ACTIVE_WRITE_GATE=storage`, then run `npm run cloudflare:functional:remote:storage`. It signs in as the synthetic guest, uploads through the unchanged `experiences` bucket RLS, verifies the public bytes, removes the object as its owner, and proves cleanup. Never point its environment at production. The broader host profile and experience-create journeys remain available through `03-live-host-signup-registration.spec.ts` and `04-live-host-experience-create.spec.ts` when those product flows are explicitly required.
