...
The producer is fail-closed and dormant. It requires all of the following server-side runtime values: exact `CLOUDFLARE_DEPLOYMENT_ENV=production`, exact `PUBLIC_EXPERIENCE_MEDIA_PRODUCER_ENABLED=true`, a valid comma-separated `PUBLIC_EXPERIENCE_MEDIA_PRODUCER_EXPERIENCE_IDS` allowlist containing the saved experience ID, and a `PUBLIC_EXPERIENCE_MEDIA_QUEUE` binding. None of these producer values or bindings is configured by this change. Browser input, `NODE_ENV`, and `NEXT_PUBLIC_*` values cannot enable the producer.
...
The current hook points are the confirmed insert/update result inside `createExperienceFromBody` and `updateExperienceFromBody`, the confirmed experience result in `updateAdminStatus`, and the compare-and-update result of the admin photo reorder route. The create path always saves `pending` and therefore does not send. Admin status changes send only when the saved row is actually `status=active` and `is_active=true`; the current dashboard's `approved` transition is not treated as public-active. No active runtime callsite directly changes `experiences.is_active`. Deletes never enqueue and never delete mirror objects.
