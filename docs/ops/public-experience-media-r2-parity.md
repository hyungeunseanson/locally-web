...
`wrangler.jsonc` retains fail-safe OFF/empty producer defaults, while `config/cloudflare/migration-manifest.json` owns the reviewed Production release profiles. The canonical deploy wrapper injects one profile into both the reader build and producer runtime, so the two lists cannot drift: no argument selects the fixed approved cohort, `--media-profile=single-3309` selects the bounded first rollout, and `--media-profile=off` creates the explicit recovery build. Dashboard-only drift and raw Wrangler invocation are not activation mechanisms.
...
Static card and detail manifests remain the read authority outside the reviewed rollout. Deterministic delivery requires both `NEXT_PUBLIC_PUBLIC_EXPERIENCE_MEDIA_READER_ENABLED=true` and an exact numeric allowlist compiled into the client. Repository-owned raw build defaults are `false` and an empty list; malformed, partial, wildcard, and non-allowlisted configurations cannot enter the deterministic path. This is a build-time flag-and-allowlist boundary, not an environment-name gate. The canonical Production deploy supplies the selected repo-owned release profile to the build and supplies the same list to the producer runtime; changing Worker runtime vars alone cannot change the compiled reader. The explicit `off` profile performs a fresh OFF build as well as an OFF runtime deploy.
...
The approved fixed cohort may be activated only after a fresh manifest-independent audit reports no missing/conflict and quota remains inside the existing safety boundary. New IDs are never included automatically. Static manifests remain the fallback authority for non-allowlisted traffic, missed-enqueue recovery remains approval-gated rather than unattended, and the preserved DLQ event still requires a separate disposition decision.
...
Objects produced by the Wave 1.2 controlled repair remain compatible with this boundary. A `legacy-observed` derivative may omit `source_size`, and a legacy immutable original may omit `sha256` and the later provenance triplet; their internally consistent source/output hashes, size, HTTP metadata, and identity are metadata proof only. They are not reported as current-byte verified until a bounded plan downloads the current source. Partial or contradictory provenance is a conflict rather than a legacy match.

A manual `plan` performs a bounded rotating verification pass. The default planning ceiling is 12 source GETs, 64 MiB of source bytes, 12 original creates, 40 derivative creates, and 40 Sharp transforms; hard ceilings remain 50 source GETs, 256 MiB, 50 originals, 200 derivatives, and 200 transforms. A transform attempt consumes its allowance before Sharp is called, including provider failures. Source GET attempts and bytes actually received—including bytes read before a stream failure—are tracked separately for plan, pre-apply verification, apply (normally zero), and post-apply verification. Before approving a source proof, planning reserves both of its required verification reads against one fixed whole-lifecycle ceiling of 50 GETs and 256 MiB; it never grows that ceiling to fit the plan. All phases update one private persisted ledger. `Content-Length` and Storage metadata are only preflight hints, while the response stream has a 10 MiB per-object limit and a bounded timeout. The caller supplies the next cursor from the prior report so repeated runs do not always examine the same prefix. Exhausted budgets produce `partial=true` with remaining metadata-only coverage; they never become “missing zero,” and a required post-apply verification cannot be skipped as success. GitHub Actions scheduling is best effort, so this is neither guaranteed delivery nor a promise of recovery within 24 hours.

`apply` is default-OFF and requires the explicit apply checkbox, the same scope/budget/cursor, and the exact fresh plan digest. Plan schema v2 has one canonical `execution` payload: scope, source and R2 snapshots, cursor, budgets, source proofs, object keys, hashes, sizes, transform specifications, and relative artifact references are all covered by that digest and are the Python executor's only authority. Reporting timestamps are deliberately non-semantic and the executor creates fresh provenance timestamps; there is no unhashed top-level object list. Python independently validates the schema, hard ceilings, namespace/specification, source-proof links, and every local file's path, size, and SHA before its first write. It rechecks the canonical public-active source snapshot and every selected source byte before writes, and rechecks the source again after writes. The R2 whole-state digest must still match immediately before apply. Any public/private transition, changed source, target conflict, pagination/read failure, budget-state mismatch, artifact mismatch, or digest mismatch fails closed.
