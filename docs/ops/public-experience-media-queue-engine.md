...
Root `keep_vars=true` retains remotely managed variables not declared in Wrangler config; it does not supersede the explicit committed producer defaults. The canonical Production deploy now resolves one reviewed repo-owned media release profile and applies its exact enabled/list pair to both the deterministic reader build and producer runtime. The default Production release profile is the fixed approved cohort; `single-3309` is the bounded first rollout and `off` is the explicit recovery profile. Wrangler and build fallbacks remain OFF/empty, no wildcard exists, and canary/Preview/local do not receive the Production Queue profile. Queue delivery after acceptance is covered by retry/DLQ, but a DB commit followed by a failed or interrupted enqueue can still be absent from both queues.

The manifest-independent scheduled completeness report is the operational missed-enqueue signal. Maintainers of the protected `production-media-sync` GitHub environment review the `public-experience-media-completeness-*` artifact for missing, conflict, unverifiable, partial, and budget fields. A non-empty finding requires a fresh bounded `plan`, cost/digest review, and a separate explicit `apply` approval; report generation does not send an alert or perform recovery by itself. The existing failed E1 remains preserved in the DLQ and is not part of that recovery flow.
