...
Manual `audit` is metadata-only and loads no R2 credentials. Manual `plan` reads R2 metadata and emits only a sanitized digest/count summary. `apply-create-only` requires the exact digest from a fresh plan and may create only the approved missing keys. It never restores from the stale bucket, quarantines, copies, deletes, or repairs metadata. Every create uses `If-None-Match: *`; a racing writer is accepted only after exact byte, header, and provenance verification.

The bounded create-only limits are 256 source objects, 256 MiB of source payload including retry bytes, 512 transform attempts, 512 new derivative keys, and 256 MiB of new bodies. Source bytes are downloaded once and reused for both sizes. Existing exact objects are skipped. Conflicts, snapshot drift, R2-state drift, budget exhaustion, pagination/read failures, or unexpected URL namespaces fail closed.
