...
| 항목 | 내용 |
| --- | --- |
| 🟡 `siteAnnouncements.ts` 안에 쉬운 운영 주석 추가 | `app/config/siteAnnouncements.ts` — 운영팀이 가이드 문서를 따로 열지 않아도 파일 안에서 바로 `enabled`, `id`, 기간, 본문을 어떻게 바꾸면 되는지 이해할 수 있도록 쉬운 한국어 설명 주석을 추가 |
...
| 항목 | 내용 |
| --- | --- |
| 🟠 데스크탑 채팅 리스트 밀도 압축 | `app/admin/dashboard/components/ChatMonitor.tsx` — 관리자 메시지 모니터링 좌측 패널의 높이를 소폭 키우고, 헤더/필터/리스트 row를 더 촘촘하게 줄여 데스크탑에서 한 화면에 더 많은 대화가 보이도록 조정. 이름/체험명/최신 메시지의 가독성은 유지하되 텍스트 크기와 간격을 낮춰 세로 공간을 덜 쓰게 정리 |
...
| 항목 | 내용 |
| --- | --- |
| 🟠 체험 등록 포함/불포함/준비물 품질 필터 추가 | `app/host/create/page.tsx`, `app/host/create/localization.ts` — 포함/불포함 항목은 두 글자 미만이거나 중복되면 추가되지 않도록 정리하고, step 5 검증에서도 너무 짧은 항목/중복 항목/너무 짧은 준비물을 막아 등록 품질 편차를 줄이도록 보강 |
| 🟠 체험 등록 제출 전 리스트 정리 강화 | `app/host/create/page.tsx` — 제출 payload에 포함/불포함/준비물을 trim 정리해서 저장하도록 맞춰, 앞뒤 공백과 중복 입력으로 인한 품질 저하를 줄임 |
| 🟠 호스트 등록 서버 검증 2차 보강 | `app/api/host/register/submit/route.ts` — 이름, 은행명, 예금주 최소 길이, 계좌번호 숫자 길이, 지원 동기 최소 길이를 추가로 검증하고 계좌번호는 서버에서 숫자만 정리해 저장하도록 정리 |
...
| 항목 | 내용 |
| --- | --- |
| 🟠 단계별 필수 입력 검증 추가 | `app/host/register/page.tsx`, `app/host/register/localization.ts` — 국적, 기본 정보, 자기소개, 신분증, 정산 계좌, 지원 동기처럼 빈약한 신청이 자주 들어올 수 있는 핵심 필드를 `다음` 버튼 단계에서 바로 막도록 보강. 형식이 필요한 생년월일/이메일/전화번호는 최소 형식 검증까지 추가 |
| 🟠 제출 전 전체 단계 재검사 추가 | `app/host/register/page.tsx` — 마지막 제출 직전에 1~7단계를 다시 확인하고, 빠진 항목이 있으면 해당 step으로 되돌려서 다시 채우게 정리 |
| 🟡 등록 가시성 스펙 보강 | `tests/e2e/97-host-register-visibility.spec.ts` — 기본 정보 step에서 비어 있는 상태로는 다음 단계로 넘어가지 않는지 얇게 확인하도록 회귀 체크를 추가 |
...
| 항목 | 내용 |
| --- | --- |
| 🟠 서비스 요청 폼 정보구조 재정리 | `app/services/request/page.tsx`, `app/context/LanguageContext.tsx` — 긴 단일 폼을 `일정과 기본 조건`, `원하는 서비스 내용`, `연락 및 결제 진행 정보` 3개 섹션 카드로 나누고, 각 섹션에 짧은 helper 문구를 추가해 처음 요청하는 고객도 어디에 무엇을 적어야 하는지 더 빨리 이해할 수 있게 정리 |
| 🟠 데스크탑 sticky 결제 요약 추가 | `app/services/request/page.tsx` — 데스크탑에서 우측 sticky 요약 패널을 추가해 예상 결제금액, 선택한 일정/인원/언어, `작성 → 결제 → 호스트 모집 → 호스트 선택` 다음 단계를 계속 보이게 유지. 모바일은 기존 단일 컬럼 흐름을 유지하면서 하단 요약 카드만 강화 |
...
| 항목 | 내용 |
| --- | --- |
| 🟠 채팅방 리스트 세로 밀도 조정 | `app/admin/dashboard/components/ChatMonitor.tsx` — 어드민 메시지 모니터링 데스크탑 좌측 리스트의 각 채팅방 row에 `min-height`를 부여하고, 체험명/최신 메시지가 실제로 2줄 영역을 안정적으로 쓰도록 정리해 목록이 많아져도 각 대화를 더 쉽게 훑어볼 수 있게 보강 |
...
| 항목 | 내용 |
| --- | --- |
| 🟠 대표 언어 설명 명확화 | `app/host/create/localization.ts` — 기존 `대표 원문 언어 / 원문` 표현을 `대표 언어 / 대표`로 정리하고, `주로 받고 싶은 게스트 언어로 작성 + 다른 언어는 AI 자동 번역 및 보정`이라는 의미가 한 번에 이해되도록 한국어/영어/일본어/중국어 안내 문구와 검증 문구를 모두 수정 |
...
| 항목 | 내용 |
| --- | --- |
| 🟡 호스트 모드 모바일 메뉴 커뮤니티 이동 | `app/components/SiteHeader.tsx` — 커뮤니티 링크를 섹션1 맨 위에서 섹션2의 프로필 설정 바로 아래로 이동 (도움말 센터 섹션 바로 위) |
...
- v3.38.98 - [Become a Host] 언어별 랜딩 이미지 폴더 구조 정리
  - `/become-a-host`가 더 이상 `ko` 이미지 경로를 하드코딩하지 않고, `app/become-a-host2/hostLandingAssets.ts` helper를 통해 `public/images/become-a-host/{desktop|mobile}/{locale}/1.png~7.png` 규칙으로 현재 locale 이미지를 선택하도록 정리했다.
  - 영어/일본어/중국어 이미지가 아직 없어도 페이지가 깨지지 않도록, locale별 파일이 없으면 서버에서 자동으로 `ko` asset으로 fallback한다.
  - `public/images/become-a-host/README.md`에 폴더 구조, 파일명 규칙, 섹션별 권장 사이즈를 남겨 이후 번역 이미지를 해당 폴더에 넣기만 하면 언어 전환에 맞춰 자동 반영되게 했다.
- v3.38.99 - [About] 언어별 소개 이미지 랜딩 구조 추가
  - `/about`은 기존 코드 기반 에디토리얼 페이지를 fallback으로 유지하면서, `app/about/aboutLandingAssets.ts` helper가 `public/images/about/{desktop|mobile}/{locale}/1.png...` 자산을 읽어 이미지 랜딩을 우선 렌더하도록 정리했다.
  - locale별 이미지가 빠져 있으면 같은 번호의 `ko` 이미지를 자동 사용하고, 이미지가 하나도 없으면 기존 `/about` 에디토리얼 페이지가 그대로 노출된다.
  - `public/images/about/README.md`에 폴더 구조와 파일명 규칙을 남겨 이후 번역 이미지를 넣기만 하면 언어 전환에 맞춰 자동 반영되게 했다.
- v3.39.00 - [Admin Team] 메모 self-email 및 stale whitelist 정리
  - `/api/admin/notify-team`의 `team_memo`, `team_memo_comment` 메일은 기존 `admin_whitelist` 수신자 외에 작성자 본인 이메일도 즉시 포함하도록 바꿨다. 인앱 알림과 `team_chat` 메일은 계속 작성자를 제외한다.
  - Playwright `globalSetup`이 테스트 시작 시 `admin_whitelist`의 `codex.%@example.com` 찌꺼기를 자동 삭제하도록 추가해, 중단된 admin e2e가 남긴 가짜 관리자 이메일이 팀 화이트리스트에 계속 쌓이지 않게 했다.
  - 새 스모크 `45-team-notify-policy.spec.ts`가 메모는 작성자 포함, 팀채팅은 작성자 제외 정책을 직접 검증한다.
- v3.39.01 - [Service Matching] 위치 기반 노출/계약 정렬
  - 서비스 호스트 노출 기준을 알림 helper와 같은 eligible-host 규칙으로 맞춰, 승인 호스트 중 의뢰 `country/city`와 같은 위치에 활성 체험이 등록된 경우에만 `/services` 잡보드, 의뢰 상세 읽기, 지원이 가능하도록 정리했다.
  - 서비스 의뢰 상세 응답은 고객 본인과 선택된 호스트에게만 `contact_name`, `contact_phone`를 반환하고, 승인된 매칭 호스트에게는 민감정보 없이 본문만 노출한다.
  - 서비스 의뢰 생성은 더 이상 `country='JP'`를 고정 저장하지 않고 도시 기준으로 국가를 정규화해 저장하며, 호스트 지원 페이지의 예상 수입은 하드코딩 계산 대신 저장된 `total_host_payout`을 사용한다.
  - 새 스모크 `48-service-visibility.spec.ts`, `49-service-request-contract.spec.ts`가 위치 기반 노출, 연락처 마스킹, 국가 정규화, 지원 페이지 예상 수입 계약을 직접 검증한다.
- v3.39.02 - [Service Matching] 생성/선택 write 원자성 hardening
  - `/api/services/requests`는 `service_requests(status='pending_payment')` 생성 후 사전 `service_bookings(status='PENDING')` 생성이 실패하면 방금 만든 request를 즉시 삭제하고, 삭제 실패 시에만 `cancelled` fallback을 남기도록 정리해 orphan `pending_payment` 의뢰를 줄였다.
  - `/api/services/select-host`는 booking 바인딩, 선택 지원서 `selected`, 나머지 지원서 `rejected`, `service_requests.status='matched'`를 순차 적용하되, 중간 실패 시 rollback하고 다른 선택 요청이 이미 승리한 경우에는 현재 request의 최종 selected host/application 기준으로 재정렬하도록 보강했다.
  - 새 스모크 `50-service-select-host-atomicity.spec.ts`가 병렬 선택 경쟁에서도 최종 상태가 한 명의 호스트로 정리되는지, 결제 예약이 없을 때 request/application 상태가 그대로 유지되는지 직접 검증한다.
- v3.39.03 - [Mobile UX] 호스트 사진 액션 / 게스트 예약 후속 / 모바일 unread 배지 정리
  - 호스트 체험 등록/수정의 대표 사진 썸네일은 모바일에서 탭 시 `사진 변경 / 사진 삭제` 액션시트를 열도록 정리했다. 데스크탑 hover 삭제 quick action은 그대로 유지하고, 등록/수정 둘 다 slot 단위 교체가 가능하다.
  - `GET /api/guest/trips`는 `hostName`, `hostAvatarUrl`, `meetingPoint`, `meetingPointI18n`를 함께 반환하고, `TripCard`는 `meeting_point_i18n` 우선으로 만나는 장소를 현 언어에 맞게 표시하며 메시지 진입 시 호스트 summary query를 같이 넘긴다.
  - `/guest/inbox`는 `hostId`만 있고 `hostName/hostAvatar`가 비어 있는 진입에서도 직접 host profile/application을 조회해 초기 헤더 아바타와 이름을 복구한다.
  - `useChat`의 기존 inquiry 텍스트 전송은 optimistic append로 바꿔 임시 말풍선을 즉시 보여주고, 성공 후 전체 메시지 재조회 없이 inquiry summary만 background refresh하도록 정리했다.
  - 모바일 guest `프로필` 탭, guest `/account` 상단 벨, host `더보기` 탭, host 메뉴 상단 벨은 모두 `NotificationContext.unreadCount` 기준 unread dot을 표시한다.
  - 새 스모크 `51-host-mobile-photo-actions.spec.ts`, `52-guest-trip-metadata.spec.ts`, `53-chat-optimistic-send.spec.ts`, `54-mobile-notification-badges.spec.ts`가 각각 모바일 사진 액션, guest inbox host bootstrap, 텍스트 전송 optimistic UI, 모바일 unread dot을 직접 검증한다.
