...
## v3.37.75 — [Admin Ledger Mobile] 모바일 밀도 개선 및 확인 모달 커스텀화

**작업일:** 2026-03-11

| 항목 | 내용 |
|------|------|
| 🔴 모바일 액션 모달 교체 | `MasterLedgerTab`의 모바일 `입금 확인`/`강제 취소`가 더 이상 브라우저 기본 `confirm()`을 쓰지 않고, 앱 스타일의 모바일 최적화 확인 모달을 사용하도록 수정 |
| 🟡 모바일 표 압축 | 모바일 Ledger 표는 구조를 유지한 채 글자, 패딩, 간격, 최소 폭을 줄이고 `예약 타입` 컬럼을 숨겨 한 화면 가독성을 개선 |
| 🟡 모바일 라벨 축약 | 모바일에서 `입금대기 → 입금`, `확정됨 → 확정`, `취소됨 → 취소`로 축약하고, 상세 패널 액션 문구도 짧게 정리 |
