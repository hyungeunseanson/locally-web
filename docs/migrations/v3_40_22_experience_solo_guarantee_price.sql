-- v3.40.22
-- Experience-level solo guarantee add-on pricing.
-- Default preserves the existing 30,000 KRW behavior while allowing hosts/admins
-- to adjust the add-on for new bookings. Existing bookings remain unchanged.

ALTER TABLE public.experiences
  ADD COLUMN IF NOT EXISTS solo_guarantee_price integer;

UPDATE public.experiences
SET solo_guarantee_price = 30000
WHERE solo_guarantee_price IS NULL;

ALTER TABLE public.experiences
  ALTER COLUMN solo_guarantee_price SET DEFAULT 30000,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conname = 'experiences_solo_guarantee_price_check'
       AND conrelid = 'public.experiences'::regclass
  ) THEN
    ALTER TABLE public.experiences
      ADD CONSTRAINT experiences_solo_guarantee_price_check
      CHECK (
        solo_guarantee_price >= 20000
        AND solo_guarantee_price <= 100000
        AND solo_guarantee_price % 1000 = 0
      );
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.bookings
      DROP CONSTRAINT bookings_solo_guarantee_refund_amount_check;
  END IF;
  ...
  ALTER TABLE public.bookings
    ...
    CHECK (
      ...
      AND solo_guarantee_refund_amount <= GREATEST(COALESCE(solo_guarantee_price, 0), 0)
    );
END $$;
