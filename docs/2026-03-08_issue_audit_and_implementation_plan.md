...
사용자 제보 이슈 5건을 기준으로 아래 영역을 점검했다.
...
- 이번 이슈 5건과 직접 연결된 오류는 아니지만, 자동 검증 베이스라인이 이미 깨져 있어 구현 단계에서 별도 정리가 필요하다.

## 핵심 요약

이번 5건은 각각 개별 버그처럼 보이지만, 실제로는 아래 3개의 구조 문제로 묶인다.
...
- `docs/gemini.md:147-155` 기준상 호스트 지원서와 체험 등록은 이미 별도 메모가 누적돼 있고, 연결 상태가 완전히 정리되지 않은 필드가 존재한다. 은행 추가 필드 설계도 같은 범주의 구조 작업이다.
...
### 5. 일본 계좌 송금 시 통장타입/지점명이 필요

#### 현재 상태

호스트 정산 관련 데이터 모델이 3필드에 고정돼 있다.

- 지원서 저장
  - `app/host/register/page.tsx:231-250`
- 지원서 UI
  - `app/host/register/components/HostRegisterForm.tsx:304-323`
- 호스트 대시보드 private tab
  - `app/host/dashboard/components/ProfileEditor.tsx:45-58`
  - `app/host/dashboard/components/ProfileEditor.tsx:349-355`
- 관리자 타입
  - `app/types/admin.ts:94-99`
- 관리자 정산 조회
  - `app/admin/dashboard/components/SettlementTab.tsx:50-55`
- 서비스 정산 CSV
  - `app/api/admin/service-bookings-csv/route.ts:69-72`
...
1. 국가별 지급 규칙을 흡수할 수 없는 최소 필드 모델로 시작했다.
2. 지원서, 프로필, 관리자 정산, CSV가 모두 같은 축소 모델을 공유한다.
3. private tab 이 read-only 이라 호스트가 정산 상세를 보완할 경로도 없다.

#### 영향 범위

- 일본 계좌로 실제 송금에 필요한 지점명/계좌종류/지점코드 입력 불가
- 관리자 정산 시 수기 확인/외부 메모 의존
- CSV와 정산 화면 모두 필드 부족
...
국가별 화면 분기 대신 “다양한 정산 필드 세트 중 필요한 것만 입력” 구조로 바꾸는 것이 맞다.
...
옵션 A. 컬럼 확장
- 장점: 쿼리/CSV/관리자 화면이 단순
- 단점: 컬럼이 늘어남

옵션 B. `payout_details` JSONB 추가
- 장점: 유연함
- 단점: 프론트/관리자/CSV에서 파싱 비용 증가

이 프로젝트 기준 권장안:

- 옵션 A + 가벼운 메타 필드
- 이유: 현재 관리자 화면과 CSV가 열 기반 UI라서 JSONB보다 유지보수가 쉽다.

필수 수정 범위:

1. DB migration
2. `host_applications` insert/update
3. 호스트 지원서 UI
4. 호스트 대시보드 private tab 표시
5. 관리자 host-applications API
6. SettlementTab
7. ServiceAdminTab / Sales CSV / service-bookings CSV
8. `app/types/admin.ts`
...
2. 계좌 유형 선택 드롭다운
3. 지점명/지점코드 입력란 제공
4. 국가별 분기 대신 필요한 필드만 펼쳐지는 보조형 UI
5. 미입력 시 관리자 화면에서 “추가 정보 필요” 뱃지 표시
...
4. 계좌 필드 확장은 UI만이 아니라 admin export/정산까지 한 번에 반영해야 한다.
5. HEIC는 일부 브라우저에서 변환 실패 후 원본이 남기 때문에, 지원 여부를 명확히 분기해야 한다.

## 결론

가장 먼저 손대야 할 것은 메시지 진입 UX다. 이미 필요한 URL 파라미터 기반 규약이 구현돼 있어서, 예약 완료 페이지와 `new_message` 알림 생성 링크만 바로잡아도 사용자 체감 문제가 크게 줄어든다.

그 다음은 i18n과 법률 문구 표시 방식 정리다. 현재는 번역 시스템이 없는 것이 아니라, 핵심 플로우 일부가 시스템 밖에 남아 있다. 마지막으로 HEIC와 일본 계좌 정산은 공통 정책과 스키마 수준의 결정이 필요한 구조 작업으로 분리해 처리하는 것이 맞다.
