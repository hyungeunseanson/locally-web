...
- 실제 결과
  ...
  - 메모
    ...
    - 다만 한 가지 audit gap이 남는다
      ...
      - 현재 Codex 세션에서는 `vercel` CLI 미설치, `VERCEL_*` auth env 부재, `~/.vercel` 설정 부재가 함께 확인됐다
      - `/tmp` 기반 임시 config/cache로 `vercel logs --help`까지는 확인했지만, 실제 `vercel logs --project locally-web --query '/api/host/register/admin-alert' --since 30d --limit 5 --json --no-follow --non-interactive` 실행은 `No existing credentials found. Please log in` device login 경계에서 멈췄다
...
- 호스트 신청/승인 경계의 non-live gap
  ...
  - 현재 Codex 세션에서도 Vercel CLI 로그 명령 자체는 확인했지만, 실제 query는 device login/auth boundary에서 멈춰 repo 밖 운영 관측으로만 닫을 수 있음이 재확인됐다
...
- 남은 후속 1건
  - host 신청/승인 경계
    ...
    - 이번 Codex 세션에서는 로컬 CLI/auth/access boundary까지 확인했으므로, 다음 액션은 구현이 아니라 운영자 권한으로 위 `vercel logs ... --query '/api/host/register/admin-alert'` runtime request log를 조회하는 것이다
