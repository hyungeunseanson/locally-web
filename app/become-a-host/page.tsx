...
export default function BecomeHostPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <main>
        {/* 1. 히어로 섹션 */}
        <section className="max-w-[1440px] mx-auto px-4 md:px-6 py-11 md:py-20 lg:py-32 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-16">
          <div className="flex-1 space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <h1 className="text-[32px] md:text-7xl font-black leading-[1.1] tracking-tight">
              좋아하는 일을 하며<br/>
              <span className="text-rose-600">수입</span>을 올리세요.
            </h1>
            <p className="text-[15px] md:text-xl text-slate-500 font-medium leading-relaxed max-w-lg">
            ...
            <div className="pt-4">
              <button 
                ...
                className="bg-gradient-to-r from-rose-500 to-rose-600 text-white px-6 md:px-10 py-3 md:py-5 rounded-2xl font-bold text-[14px] md:text-xl hover:shadow-xl hover:scale-105 transition-all duration-300 flex items-center gap-1.5 md:gap-2"
              >
          ...
          <div className="flex-1 flex justify-center md:justify-end relative">
             <div className="relative w-[262px] h-[520px] md:w-[340px] md:h-[680px] bg-black rounded-[44px] md:rounded-[60px] border-[9px] md:border-[12px] border-slate-900 shadow-2xl overflow-hidden ring-1 ring-slate-900/5">
        ...
        <section className="bg-white py-14 md:py-32">
          <div className="max-w-[1440px] mx-auto px-4 md:px-6">
            <h2 className="text-[21px] md:text-5xl font-black text-center mb-9 md:mb-24 leading-tight">
              어디서도 만나볼 수 없는<br/>독특한 체험을 호스팅하세요
            </h2>
            <div className="grid md:grid-cols-3 gap-x-8 gap-y-12 md:gap-y-16">
        ...
        {/* 3. 모바일 목업 섹션 */}
        <section className="bg-slate-50 py-14 md:py-32">
          <div className="max-w-[1440px] mx-auto px-4 md:px-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-10 md:gap-20 mb-14 md:mb-32">
               ...
               <div className="flex-1 order-1 md:order-2">
                  <h3 className="text-[27px] md:text-3xl font-black mb-4 md:mb-6">게스트와 간편한 소통</h3>
                  <p className="text-[16px] md:text-xl text-slate-500 leading-relaxed font-medium">앱 내 채팅 기능을 통해 전 세계 게스트와 실시간으로 대화하세요.<br/>개인 연락처 노출 걱정 없이 안전하게 소통할 수 있습니다.</p>
               </div>
            </div>
            <div className="flex flex-col md:flex-row items-center justify-between gap-10 md:gap-20">
               <div className="flex-1">
                  <h3 className="text-[27px] md:text-3xl font-black mb-4 md:mb-6">투명하고 신속한 정산</h3>
                  <p className="text-[16px] md:text-xl text-slate-500 leading-relaxed font-medium">체험이 완료되면 다음 달 바로 입금됩니다.<br/>복잡한 절차 없이 수익을 확인하고 관리하세요.</p>
               </div>
               <div className="flex-1 flex justify-center">
                  <div className="relative w-[292px] md:w-[320px] bg-white rounded-[34px] md:rounded-[40px] shadow-2xl p-6 md:p-8 border border-slate-100 transform rotate-[2deg] hover:rotate-0 transition-transform duration-500 text-center">
                     <div className="text-slate-500 font-bold mb-2">5월 정산 예정 금액</div>
                     <div className="text-[34px] md:text-5xl font-black mb-6 md:mb-8 tracking-tight">₩499,784</div>
                     <div className="space-y-4">
        ...
        {/* 4. FAQ 및 CTA */}
        <section className="py-14 md:py-32 max-w-4xl mx-auto px-4 md:px-6">
          <h2 className="text-[21px] md:text-4xl font-black mb-8 md:mb-16 text-center">자주 묻는 질문</h2>
          ...
          <div className="mt-12 md:mt-32 text-center bg-black rounded-[2.5rem] md:rounded-[3rem] p-7 md:p-16 text-white relative overflow-hidden">
             <div className="relative z-10">
                <h2 className="text-[22px] md:text-5xl font-black mb-5 md:mb-8">지금 바로 시작해보세요</h2>
                <p className="text-slate-400 text-[13px] md:text-lg mb-6 md:mb-10">당신의 평범한 하루가 누군가에게는 잊지 못할 추억이 됩니다.</p>
                <button 
                  onClick={handleStartClick}
                  className="bg-white text-black px-7 md:px-12 py-3 md:py-5 rounded-2xl font-bold text-[14px] md:text-xl hover:scale-105 transition-transform flex items-center gap-1.5 md:gap-2 mx-auto"
                >
...
function FeatureItem({ icon, title, desc }: { icon: React.ReactElement<{ className?: string }>; title: string; desc: string }) {
  return (
    <div className="flex flex-col items-start group">
      <div className="mb-5 md:mb-6 p-3 md:p-4 rounded-2xl bg-slate-50 group-hover:bg-rose-50 transition-colors duration-300">
        {React.cloneElement(icon, { className: "w-7 h-7 md:w-8 md:h-8 text-slate-900 group-hover:text-rose-600 transition-colors" })}
      </div>
      <h3 className="text-[20px] md:text-2xl font-bold mb-2 md:mb-3 leading-tight">{title}</h3>
      <p className="text-slate-500 leading-relaxed text-[15px] md:text-lg">{desc}</p>
    </div>
  );
}
...
function FAQItem({ q, a }: { q: string, a: string }) {
  ...
  return (
    <div className="border-b border-slate-100 last:border-0">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center py-6 md:py-8 text-left hover:text-rose-600 transition-colors">
        <span className="font-bold text-[18px] md:text-2xl">{q}</span>
        {isOpen ? <ChevronUp size={24}/> : <ChevronDown size={24}/>}
      </button>
      {isOpen && <div className="pb-6 md:pb-8 text-slate-500 leading-relaxed text-[15px] md:text-lg">{a}</div>}
    </div>
  );
}
