  ...
  try {
    ...
    if (hasDateFilter && filteredServiceRequests && filteredServiceRequestIds.length === 0) {
      ...
      let serviceBookingsQuery = supabaseAdmin
        ...
        .select(
          'id, order_id, request_id, application_id, customer_id, host_id, amount, created_at, status, payment_method, host_payout_amount, host_compensation_amount, refund_amount, platform_revenue, payout_status'
        )
        .order('created_at', { ascending: false });
      if (hasDateFilter && filteredServiceRequests && filteredServiceRequestIds.length > 0) {
