...
export default function ListPanel({
  ...
  return (
    <div className="flex-1 bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col shadow-sm min-w-[320px]">
      ...
      {/* 🟢 필터 헤더 */}
      <div className="p-3 md:p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center sticky top-0 z-10 backdrop-blur-sm">
        ...
        {listItems.map((item: any) => {
          ...
          return (
            <div
              ...
              className={`p-3 md:p-4 rounded-xl border cursor-pointer transition-all flex gap-3 md:gap-4 items-center ${selectedItem?.id === item.id ? 'border-slate-900 bg-slate-50 ring-1 ring-slate-900' : 'border-slate-100 hover:border-slate-300 hover:bg-white bg-white'}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between mb-1">
                  <div className="font-bold text-xs md:text-sm truncate text-slate-900 pr-2">{item.title || item.name || item.full_name || 'Unknown'}</div>
                  <span className={`text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0 ${activeTab === 'USERS' ? 'bg-slate-100 text-slate-500' : item.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : item.status === 'approved' || item.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
