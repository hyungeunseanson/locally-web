...
const BASE_PREFETCH_LIMIT = 8;
const SUMMARY_SORT_PREFETCH_LIMIT = 12;
...
function getUserRoleBadgeClassName(role?: string | null) {
  if (role === 'admin') return 'bg-rose-100 text-rose-700 border border-rose-200';
  if (role === 'host') return 'bg-violet-100 text-violet-700 border border-violet-200';
      ...
      <div className={`flex-1 flex overflow-y-auto p-1 animate-in fade-in zoom-in-95 duration-300 ${selectedUser ? 'hidden md:flex flex-col md:w-2/3 md:pr-4' : 'flex-col w-full'}`}>
        ...
        <section className="bg-white rounded-lg md:rounded-2xl border border-slate-200 p-3 md:p-6 mb-3 md:mb-6 shadow-sm shrink-0">
          ...
          {onlineUsers.filter((u: OnlineUser) => !u.is_anonymous).length > 0 ? (
            ...
            <div className="text-sm text-slate-400 py-2">현재 접속 중인 회원이 없습니다.</div>
          )}
        </section>
            ...
            <table className="w-full text-xs md:text-sm text-left min-w-[900px]">
              <thead className="text-[10px] md:text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 md:px-6 py-2 md:py-3">회원 정보</th>
                ...
                {visibleUsers.map((user: AdminUserDashboardRow) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2 md:px-6 py-2.5 md:py-4">
                        <span className={`px-1.5 md:px-2 py-0.5 md:py-1 rounded text-[8px] md:text-[10px] font-bold ${getUserRoleBadgeClassName(user.role)}`}>
      ...
      {selectedUser && (
        <div ref={detailPanelRef} className="absolute inset-0 z-[100] md:z-30 md:relative md:w-[450px] md:self-start border-l border-slate-200 bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 right-0 top-0">
          ...
          <div className="p-3 md:p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <div>
              <h3 className="font-bold text-sm md:text-lg text-slate-900">{getUserRoleLabel(selectedUser.role)}</h3>
          ...
          <div ref={detailScrollRef} className="flex-1 overflow-y-auto">
            ...
            <div className="p-4 md:p-6 border-b border-slate-100">
              <h4 className="text-[10px] md:text-xs font-bold text-slate-900 uppercase mb-3 md:mb-4">기본 프로필</h4>
