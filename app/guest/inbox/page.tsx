...
export default function GuestInboxPage() {
  ...
  const [currentUserId, setCurrentUserId] = useState<string | null>(null); // ID만 따로 관리

  useEffect(() => {
    const fetchInquiries = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) { router.push('/'); return; }
      setCurrentUserId(user.id); // ID 저장
  ...
  const handleSend = async () => {
    ...
    const { error } = await supabase.from('inquiry_messages').insert([{
      inquiry_id: selectedInquiry.id, sender_id: currentUserId, content: inputText
    }]);
            ...
            {selectedInquiry ? (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/30">
                  ...
                  {/* 대화 */}
                  {messages.map((msg) => {
                    const isMe = msg.sender_id === currentUserId; // ID 직접 비교
                    return (
                      <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`p-3 rounded-2xl max-w-[70%] text-sm shadow-sm ${isMe ? 'bg-black text-white rounded-tr-none' : 'bg-white border border-slate-200 rounded-tl-none'}`}>
                          {msg.content}
                        </div>
                      </div>
                    );
                ...
                <div className="p-4 border-t border-slate-100 flex gap-3 bg-white">
                  <input className="flex-1 bg-slate-100 rounded-xl px-4 py-3 outline-none focus:ring-1 focus:ring-black" placeholder="메시지 입력..." value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={handleKeyDown}/>
