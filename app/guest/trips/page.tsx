      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <TripCard 
              id={1} image="https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e" title="기모노 입고 다도 체험" date="2025년 12월" host="Sakura"
              onReviewClick={handleOpenReview}
            />
            <TripCard 
              id={2} image="https://images.unsplash.com/photo-1551632811-561732d1e306" title="홋카이도 설국 스키 레슨" date="2025년 1월" host="Yuki" isReviewed={true}
              onReviewClick={handleOpenReview}
            />
    ...
    <div className="border border-slate-200 rounded-2xl overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full">
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="mt-auto pt-3 border-t border-slate-100 flex justify-between items-center">
          <div className="flex items-center gap-1 text-green-600 text-xs font-bold"><CheckCircle2 size={14}/> 이용 완료</div>
          {isReviewed ? (
            <button disabled className="text-xs font-bold text-slate-400 px-3 py-1.5 bg-slate-100 rounded-lg cursor-default">작성 완료</button>
          ) : (
            <button onClick={() => onReviewClick({ id, title, host, image })} className="text-xs font-bold text-white px-3 py-1.5 bg-black rounded-lg hover:bg-slate-800 transition-colors flex items-center gap-1"><PenTool size={12}/> 후기 작성</button>
          )}
        </div>
      </div>
    </div>
      ...
      <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-8">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-16 h-16 rounded-xl bg-slate-200 overflow-hidden shrink-0"><img src={trip.image} className="w-full h-full object-cover"/></div>
            <div><h4 className="font-bold text-sm text-slate-900 line-clamp-1">{trip.title}</h4><p className="text-xs text-slate-500 mt-1">{trip.host} 호스트님과의 만남은 어떠셨나요?</p></div>
          </div>
          <div className="flex justify-center gap-2 mb-4">
            {[1, 2, 3, 4, 5].map((star) => (
              <button key={star} onMouseEnter={() => setHoverRating(star)} onMouseLeave={() => setHoverRating(0)} onClick={() => setRating(star)} className="transition-transform hover:scale-110 p-1">
                <Star size={32} fill={(hoverRating || rating) >= star ? "#FBBF24" : "none"} className={(hoverRating || rating) >= star ? "text-amber-400" : "text-slate-300"} />
              </button>
            ))}
          </div>
          <p className="text-center text-sm font-bold text-slate-700 mb-8">{rating === 5 ? "최고였어요! 😍" : rating === 4 ? "좋았어요! 😊" : rating === 3 ? "보통이에요 🙂" : rating === 2 ? "아쉬웠어요 🙁" : rating === 1 ? "별로였어요 😫" : "별점을 눌러 평가해주세요"}</p>
          <textarea className="w-full h-32 p-4 border border-slate-300 rounded-xl resize-none focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all text-sm mb-4" placeholder="솔직한 후기를 남겨주세요. (최소 10자 이상)" value={reviewText} onChange={(e) => setReviewText(e.target.value)}/>
          
          <div className="flex gap-3 mb-6">
            {images.map((img, idx) => (
              <div key={idx} className="relative w-16 h-16 rounded-lg overflow-hidden border border-slate-200 group">
                <img src={img} className="w-full h-full object-cover"/>
                <button onClick={() => removeImage(idx)} className="absolute top-0 right-0 bg-black/50 text-white p-0.5 rounded-bl-lg opacity-0 group-hover:opacity-100 transition-opacity"><X size={12}/></button>
              </div>
            ))}
            {images.length < 2 && (
          ...
          <button onClick={handleSubmit} className="w-full bg-black text-white font-bold py-4 rounded-xl hover:bg-slate-800 transition-colors shadow-lg active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed" disabled={rating === 0 || reviewText.length < 10}>후기 등록하기</button>
        </div>
      </div>
    </div>
  );
