...
export default function LoginModal({ isOpen, onClose, onLoginSuccess }: LoginModalProps) {
        ...
        if (data.user) {
          const { error: profileError } = await supabase
            ...
            .upsert({
              ...
              email: email,
              full_name: fullName,
              phone: phone, // DB 컬럼명 확인 필요 (phone 또는 phone_number)
          ...
          if (profileError) {
            ...
            // 가입은 됐으므로 에러를 띄우진 않고 로그만 남김 (혹은 토스트 경고)
          }
        }
  ...
  }, [email, password, mode, fullName, phone, birthDate, gender, supabase, router, onClose, onLoginSuccess, showToast]);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <div className={`bg-white w-full ${mode === 'SIGNUP' ? 'max-w-[480px]' : 'max-w-[420px]'} rounded-2xl shadow-2xl overflow-hidden relative z-10 animate-in zoom-in-95 duration-200 transition-all`}>
        ...
        <div className={`p-6 ${mode === 'SIGNUP' ? 'max-h-[80vh] overflow-y-auto' : ''}`}>
          ...
          {/* 🟢 입력 그룹 (Unified Input Group) */}
          <div className="border border-gray-300 rounded-xl overflow-hidden mb-6">
            
            {/* 이메일 (공통) */}
            <InputItem 
              type="email" label="이메일" value={email} setValue={setEmail} 
              isFirst={true} isLast={mode === 'LOGIN' ? false : false} // 로그인일땐 중간, 가입일땐 첫번째
              onKeyDown={handleKeyDown} focusKey="EMAIL" currentFocus={isFocused} setFocus={setIsFocused}
            />

            {/* 비밀번호 (공통) */}
            <InputItem 
              type="password" label="비밀번호" value={password} setValue={setPassword} 
              isFirst={false} isLast={mode === 'LOGIN' ? true : false} // 로그인일땐 마지막
              onKeyDown={handleKeyDown} focusKey="PASSWORD" currentFocus={isFocused} setFocus={setIsFocused}
            />
            ...
            {mode === 'SIGNUP' && (
              <>
                <InputItem 
                  ...
                  onKeyDown={handleKeyDown} focusKey="NAME" currentFocus={isFocused} setFocus={setIsFocused}
                />
                
                <InputItem 
                  type="tel" label="휴대폰 번호 (- 없이 입력)" value={phone} setValue={setPhone} 
                  isFirst={false} isLast={false} 
                  onKeyDown={handleKeyDown} focusKey="PHONE" currentFocus={isFocused} setFocus={setIsFocused}
                />
                ...
                <div className="flex border-t border-gray-300">
                   ...
                   <div className={`relative h-14 w-1/2 border-r border-gray-300 ${isFocused === 'BIRTH' ? 'ring-2 ring-black z-10' : ''}`}>
                      <input
                        type="text" // date 타입은 스타일링이 어려워 text로 받고 placeholder 가이드
                        ...
                        value={birthDate}
                        onChange={(e) => {
                           const val = e.target.value.replace(/[^0-9]/g, '').slice(0, 8); // 숫자만, 8자리 제한
                           setBirthDate(val);
                        }}
          ...
          <div className="mt-6 text-center text-sm">
             <button 
                onClick={() => {
                  ...
                  setIsFocused(null); // 모드 전환 시 포커스 초기화
                }} 
...
// 🧱 하위 컴포넌트: 입력 아이템 (스타일 통일)
function InputItem({ type, label, value, setValue, isFirst, isLast, onKeyDown, focusKey, currentFocus, setFocus }: any) {
