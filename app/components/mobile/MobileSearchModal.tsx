...
export default function MobileSearchModal({
    ...
    return (
        <div className="fixed inset-0 z-[200] flex flex-col">
            {/* 배경: warm beige/pink 그라데이션 + 블러 */}
            <div
                className="absolute inset-0 -z-10 backdrop-blur-[8px] transition-opacity duration-300"
                style={{
                    background: 'linear-gradient(160deg, rgba(245,238,232,0.97) 0%, rgba(240,232,226,0.95) 30%, rgba(235,228,222,0.93) 60%, rgba(240,235,230,0.92) 100%)',
            ...
            >
                {/* 🔝 상단: 아이콘 탭 중앙정렬 + X */}
                <div className="flex items-center justify-between px-5 pt-[calc(env(safe-area-inset-top,0px)+12px)] pb-3">
                    {/* 탭 — 중앙 배치를 위한 flex-1 + justify-center */}
                    <div className="flex-1 flex items-center justify-center gap-[40px]">
                        ...
                        <button onClick={() => setActiveTab('experience')} className="flex flex-col items-center relative active:scale-[0.90] transition-transform duration-200">
                            <div className="w-[40px] h-[40px] flex items-center justify-center relative mb-[1px]">
                                <img
                            ...
                            {activeTab === 'experience' && <span className="absolute -bottom-[2px] left-1/2 -translate-x-1/2 w-[16px] h-[2px] bg-[#222222] rounded-full" />}
                        </button>
                        {/* 서비스 */}
                        <button onClick={() => setActiveTab('service')} className="flex flex-col items-center relative active:scale-[0.90] transition-transform duration-200">
                            ...
                            {activeTab === 'service' && <span className="absolute -bottom-[2px] left-1/2 -translate-x-1/2 w-[16px] h-[2px] bg-[#222222] rounded-full" />}
                        </button>
                    </div>
                    ...
                    <button
                        onClick={handleClose}
                        className="w-[28px] h-[28px] rounded-full flex items-center justify-center bg-white shrink-0 active:scale-[0.9] transition-transform ml-3"
                ...
                <div className="flex-1 overflow-y-auto px-5 pt-2 pb-28">
                    ...
                    {activePanel === 'location' ? (
                        <div
                            ...
                            style={{ borderRadius: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04), 0 4px 16px rgba(0,0,0,0.06)', border: '0.5px solid #E8E8E8' }}
                        >
                            <div className="p-5 pb-4">
                                <h3 className="text-[16px] font-extrabold text-[#222222] mb-4 tracking-[-0.02em]">위치</h3>
                                <div className="flex items-center gap-2.5 bg-[#F7F7F7] rounded-[10px] px-3.5 py-[11px]" style={{ border: '1px solid #E0E0E0' }}>
                                ...
                                <div className="mt-4">
                                    <p className="text-[10px] font-semibold text-[#717171] mb-2 tracking-[0.04em]">최근 검색</p>
                                    {CATEGORIES.filter(c => c.id !== 'all').slice(0, 5).map((city) => (
                                        <button
                                            key={city.id}
                                            onClick={() => { setLocationInput(t(city.label)); setActivePanel('date'); }}
                                            className="flex items-center gap-3 w-full py-[7px] text-left active:bg-[#F7F7F7] rounded-lg transition-colors"
                                        >
                                            <div
                                            ...
                                            <span className="text-[13px] font-normal text-[#222222]">{t(city.label)}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                    ...
                    {activePanel === 'date' ? (
                        ...
                        >
                            <div className="p-5">
                                <h3 className="text-[16px] font-extrabold text-[#222222] mb-4 tracking-[-0.02em]">날짜</h3>
                                <DatePicker
                    ...
                    {activePanel === 'language' ? (
                        ...
                        >
                            <div className="p-5">
                                <h3 className="text-[16px] font-extrabold text-[#222222] mb-4 tracking-[-0.02em]">언어</h3>
                                <div className="space-y-0.5">
                                    {languages.map((lang) => (
                                        ...
                                        >
                                            <div className="w-[26px] h-[18px] rounded-[3px] overflow-hidden flex items-center justify-center bg-[#F7F7F7] border border-[#EBEBEB]">
                                                {lang.icon ? (
                                                    <span className="text-[11px]">{lang.icon}</span>
                                                ) : (
