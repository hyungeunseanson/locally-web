...
import React, { useEffect, useState, Suspense } from 'react'; // ✅ Suspense 추가
import { CheckCircle, Home, FileText, Loader2 } from 'lucide-react';
import Link from 'next/link';
import { useSearchParams } from 'next/navigation';
import { supabase } from '@/app/lib/supabase';

// 1. 기존 로직을 담은 실제 컨텐츠 컴포넌트
  ...
  return (
    <div className="bg-white max-w-md w-full rounded-2xl shadow-xl p-8 text-center">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link href="/" className="flex-1"><button className="w-full py-3 border border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-50 flex items-center justify-center gap-2"><Home size={18}/> 홈으로</button></Link>
        <Link href="/guest/trips" className="flex-1"><button className="w-full py-3 bg-black text-white rounded-xl font-bold hover:scale-105 transition-transform flex items-center justify-center gap-2"><FileText size={18}/> 내 여행 보기</button></Link>
      </div>
    </div>
  );
}
