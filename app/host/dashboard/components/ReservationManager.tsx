...
import { 
  Calendar, Clock, User, CheckCircle2, XCircle, MessageSquare, MoreHorizontal 
} from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {filteredList.length === 0 ? (
          ...
          filteredList.map(res => (
            <div key={res.id} className="border border-slate-100 rounded-xl p-5 hover:border-slate-300 transition-colors bg-white">
              
              {/* 상단: 게스트 정보 */}
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden border border-slate-200 flex items-center justify-center text-slate-400">
                    <User size={18}/>
                  </div>
                  <div>
