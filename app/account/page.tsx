...
import { User, ShieldCheck, Star, Save } from 'lucide-react';
...
export default function AccountPage() {
  ...
  useEffect(() => {
    const getProfile = async () => {
      ...
      // 프로필 테이블에서 데이터 가져오기 (없으면 Auth 메타데이터 사용)
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('profiles').upsert({
      ...
      birth_date: profile.birth_date || null, // 빈 문자열이면 null
      bio: profile.bio,
      phone: profile.phone,
      email: user.email // 이메일은 업데이트용으로 저장
    });
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="w-full lg:w-[340px] flex-shrink-0">
            <div className="border border-slate-200 rounded-3xl p-8 shadow-sm text-center sticky top-28">
              <div className="w-32 h-32 bg-slate-200 rounded-full mx-auto mb-4 overflow-hidden border border-slate-100 relative">
              ...
              <h2 className="text-xl font-bold mb-1">{profile.full_name || '이름 없음'}</h2>
              <div className="flex items-center justify-center gap-2 text-sm text-slate-500 mb-6">
                <ShieldCheck size={14} className="text-black"/> 신원 인증 완료
              </div>
              
              <div className="text-left space-y-4 pt-6 border-t border-slate-100">
                <h3 className="font-bold text-lg">호스트가 받은 후기 ({reviews.length})</h3>
                {reviews.map(review => (
                  <div key={review.id} className="bg-slate-50 p-4 rounded-xl text-sm">
                    <div className="flex justify-between mb-1">
                      <span className="font-bold">{review.host}님</span>
