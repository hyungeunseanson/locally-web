...
test.describe('dormant public experience media Queue mirror engine', () => {
  ...
  test('contains no overwrite, Copy, Delete, queue wiring, or runtime activation path', () => {
    ...
    expect(workerSource).not.toContain('queue(');
    expect(wranglerSource).not.toContain('queues');
    expect(wranglerSource).not.toContain('PUBLIC_EXPERIENCE_IMAGES_R2');
