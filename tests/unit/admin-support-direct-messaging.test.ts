...
test('uses one role-aware delivery resolver for initial and follow-up support messages', () => {
  ...
  assert.match(inquirySource, /recipientIsAdmin: !actorIsAdmin/);
  assert.match(inquirySource, /recipientIsAdmin: contextType !== 'admin_initiated_support'/);
  assert.match(
    inquirySource,
...
test('treats every support-side participant as an authenticated admin', () => {
  assert.match(
    inquirySource,
    /!isParticipant \|\| \(isAdminSupport && String\(inquiry\.user_id\) !== String\(actor\.id\)\)/
  );
